<div class="card-body" id="materials-list">
    <table
        @if(isset($item->specifications) && count($item->specifications)) id="simpleDataTableCustom"
        @endif class="table display">
        <thead>
        <tr>
            <th>SL.</th>
            <th>Entry Date</th>
        ...
        @if(isset($item->specifications) && count($item->specifications))
            ...
            @foreach($item->specifications as $tm)
                <tr>
                    ...
                    <td>{!! $tm->asset->materials_name !!}
                        ({!! $tm->asset->recourse_code !!})
                    </td>
                    ...
                    <td>
                        <button class="text-success border-0 inline-block bg-none"
                                ref="{!! $tm->id !!}"
                                onclick="return Obj.fixedAssetTransferSpecEdit(this)"><i
                                class="fas fa-edit"></i></button>
                        <button class="text-danger border-0 inline-block bg-none"
                                ref="{!! $tm->id !!}"
                                onclick="return Obj.deleteFixedAssetTransferSpec(this)">
